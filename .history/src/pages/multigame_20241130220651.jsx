...
export default function MultiGame() {
  ...
  const handleCardClick = (cardId) => {
    // Prevent clicking if the card is already flipped or matched, or if it's not the player's turn
    if (
      flippedCards.length === 2 ||
      cards.find((card) => card.id === cardId).matched ||
      (currentPlayer === 1 && player1StartTime === null) ||
      (currentPlayer === 2 && player2StartTime === null) ||
      (currentPlayer !== 1 && currentPlayer !== 2) // Prevent interaction when it's not the player's turn
    ) {
      return;
    }
