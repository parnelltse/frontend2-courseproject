...
export default function MultiGame() {
  ...
  const [gameStopped, setGameStopped] = useState(false); // Track if the game has stopped
  ...
  const initializeGame = () => {
    ...
    setGameEnded(false);
    setMatchedPairs(0);
    setGameStopped(false); // Reset the game stop flag when a new game starts
  };
  ...
  const handleCardClick = (cardId) => {
    if (gameStopped) return; // Don't allow clicks if the game is stopped

    const card = cards.find((card) => card.id === cardId);
    if (card.flipped || card.matched || flippedCards.length === 1) return;
