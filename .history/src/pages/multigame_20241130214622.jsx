...
export default function MultiGame() {
  ...
  const [gameStopped, setGameStopped] = useState(false); // To stop the game after one match
  ...
  const initializeGame = () => {
    ...
    setGameEnded(false);
    setMatchedPairs(0);
    setGameStopped(false); // Enable flipping again when the game restarts
  };
  ...
  const handleCardClick = (cardId) => {
    // Prevent clicks if the game is stopped or if the card is already flipped/matched
    if (gameStopped || flippedCards.length === 2 || cards.find((card) => card.id === cardId).matched) return;
    ...
    if (flippedCards.length === 1) {
      ...
      if (firstCard.image === secondCard.image) {
        ...
        setTimeout(() => {
          ...
          setGameStopped(true); // Stop the game after one match
        ...
        setTimeout(() => {
          ...
          setCards(updatedCards);
          setGameStopped(true); // Stop the game after a mismatch
